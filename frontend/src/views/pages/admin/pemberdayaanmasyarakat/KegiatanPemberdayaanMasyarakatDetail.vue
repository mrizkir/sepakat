<template>
  <AdminLayout>
    ...
    <v-container fluid v-if="Object.keys(data_kegiatan).length"> 
      <v-row>
        <v-col cols="12">
          <DK :datakegiatan="data_kegiatan" path="/kegiatan/pemberdayaanmasyarakat"/>
        </v-col>
      </v-row>
      <v-row>
        <v-col cols="12">
          <v-bottom-navigation
            ...
            <v-btn :to="{path: '/kegiatan/pemberdayaanmasyarakat/' + kegiatan_id + '/files'}">
              <span>Files</span>
  ...
  created() {
    ...
    this.breadcrumbs = [
      ...
      },
      {
        text: 'NEGOISASI',
        disabled: false,
        href: '/kegiatan/pemberdayaanmasyarakat/'
      },
      {
        text: 'DETAIL',
        disabled: true,
  ...
  methods: {
    ...
    verifikasi() {
      this.$root.$confirm.open('Verifikasi', 'Setelah diverifikasi, tidak bisa diberi komentar, diubah, atau dihapus ?', { color: 'green',width:600 }).then(confirm => {
        if (confirm) {
          this.btnLoading = true
          this.$ajax.post('/kegiatan/pemberdayaanmasyarakat/verifikasi/' + this.kegiatan_id,
            {
              _method: 'put',
            },
            {
