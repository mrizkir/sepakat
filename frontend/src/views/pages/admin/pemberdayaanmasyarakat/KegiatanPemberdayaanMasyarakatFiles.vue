<template>
  <AdminLayout>
    ...
    <v-container fluid v-if="Object.keys(data_kegiatan).length">
      <v-row>
        <v-col cols="12">
          <DK :datakegiatan="data_kegiatan"  :path="'/kegiatan/pemberdayaanmasyarakat/' + kegiatan_id + '/detail'" />
        </v-col>
      </v-row>
      <v-row>
...
<script>
  ...
  import DK from '@/views/pages/admin/pemberdayaanmasyarakat/DataKegiatanPemberdayaanMasyarakat';
  export default {
    ...
    created() {
      ...
      this.breadcrumbs = [
        ...
        },
        {
          text: 'NEGOISASI',
          disabled: false,
          href: '/kegiatan/pemberdayaanmasyarakat/' + this.kegiatan_id + '/detail'
        },
        {
          text: 'FILES',
          disabled: true,
    ...
    methods: {
      initialize: async function() {
        await this.$ajax.get('/kegiatan/pemberdayaanmasyarakat/' + this.kegiatan_id, {
          headers: {
      ...
      async uploadKtpPemohon() {
        if (this.$refs.frmuploadktp.validate())
        {
          if (typeof this.filektppemohon !== 'undefined' && this.filektppemohon !== null){
            ...
            await this.$ajax.post('/kegiatan/pemberdayaanmasyarakat/uploadktppemohon/' + this.kegiatan_id,formdata,
              {
                headers: {
                  Authorization: this.$store.getters['auth/Token'],
      ...
      async uploadDaftarHadir()
      {
        if (this.$refs.frmuploaddaftarhadir.validate())
        {
          if (typeof this.filedaftarhadir !== 'undefined' && this.filedaftarhadir !== null )
          {
            ...
            await this.$ajax.post('/kegiatan/pemberdayaanmasyarakat/uploaddaftarhadir/' + this.kegiatan_id,formdata,
              {
                headers: {
                  Authorization: this.$store.getters['auth/Token'],
                  'Content-Type': 'multipart/form-data',
      ...
      async uploadDokumentasiKegiatan() {
        if (this.$refs.frmuploaddokumentasikegiatan.validate()) {
          if (typeof this.filedokumentasikegiatan !== 'undefined' && this.filedokumentasikegiatan !== null )
          {
            ...
            await this.$ajax
              .post(
                '/kegiatan/pemberdayaanmasyarakat/uploaddokumentasikegiatan/' +
                  this.kegiatan_id,
                formdata,
                {
                  headers: {
