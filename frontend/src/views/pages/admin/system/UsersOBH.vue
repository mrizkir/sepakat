    ...
    <AdminLayout>
        <ModuleHeader>
            ...
            <template v-slot:name>
                USERS ORGANISASI BANTUAN HUKUM
            </template>
            ...
            <template v-slot:desc>
                <v-alert                                        
                    ...
                    type="info"
                    >
                     User dengan role OBH bertanggungjawab terhadap proses konsultasi hukum.
                </v-alert>
            </template>
        </ModuleHeader>      
        <v-container fluid>  
                    ...
                    >
                        <template v-slot:top>
                            <v-toolbar flat color="white">
                                <v-toolbar-title>DAFTAR USERS OBH</v-toolbar-title>
                                <v-divider
                                    class="mx-4"
                                    inset
                                    vertical
                                ...
                                <v-btn color="primary"
                                    ...
                                    :loading="btnLoading"
                                    :disabled="btnLoading"
                                    @click.stop="showDialogTambahUserOBH">
                                    TAMBAH
                                </v-btn>
...
export default {
    ...
    methods: {
        initialize: async function() 
        {
            ...
            await this.$ajax.get('/system/usersobh', {
                headers: {
        ...
        showDialogTambahUserOBH: async function()
        {
        ...
        save () {
            if (this.$refs.frmdata.validate())
            {
                ...
                {
                    this.$ajax.post('/system/usersobh/' + this.editedItem.id,
                        {
                            '_method': 'PUT',
                    ...
                    this.$ajax.post('/system/usersobh/store',
                        {
                            name:this.editedItem.name,
                            email:this.editedItem.email,
                            nomor_hp:this.editedItem.nomor_hp,
        ...
        deleteItem (item) {       
            this.$root.$confirm.open('Delete', 'Apakah Anda ingin menghapus username '+item.username+' ?', { color: 'red' }).then((confirm) => {
                ...
                {
                    this.btnLoading = true
                    this.$ajax.post('/system/usersobh/'+item.id,
                        {
                            '_method': 'DELETE',
                        },
                        {
    ...
    computed: {
        formTitle () {
            return this.editedIndex === -1 ? 'TAMBAH USER OBH' : 'EDIT USER OBH'
        },
        ...mapGetters('auth', {        
